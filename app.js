...
const loadGithubExcelBtn = document.getElementById("load-github-excel");
...
const loadGithubMembersBtn = document.getElementById("load-github-members");
...
const loadGithubLogoBtn = document.getElementById("load-github-logo");
...
const ADMIN_PASSWORD = "admin40102773";
...
function loadLogoFromGithub() {
  const url = prompt("Masukkan URL gambar logo dari GitHub (raw URL):");
  if (!url) {
    return;
  }
  fetch(url)
    ...
    .then((blob) => {
      const file = new File([blob], "logo-github", { type: blob.type || "image/png" });
      handleLogoFile(file);
    })
    .catch(() => {
      showToast("Gagal mengunduh logo dari GitHub.");
    });
}
...
function applyWorkbookToBooks(workbook) {
  const firstSheetName = workbook.SheetNames[0];
  const sheet = workbook.Sheets[firstSheetName];
  const rows = XLSX.utils.sheet_to_json(sheet, { defval: "" });
...
function loadBooksFromGithubExcel() {
  const url = prompt("Masukkan URL raw file Excel dari GitHub:");
  if (!url) {
    return;
  }
  fetch(url)
    ...
    .then((buffer) => {
      try {
        ...
        const workbook = XLSX.read(data, { type: "array" });
        applyWorkbookToBooks(workbook);
        showToast("Data buku berhasil dimuat dari GitHub.");
      } catch (err) {
        showToast("Gagal membaca Excel dari GitHub.");
      }
    })
    .catch(() => {
      showToast("Gagal mengunduh Excel dari GitHub.");
    });
}
...
function applyWorkbookToMembers(workbook) {
  const firstSheetName = workbook.SheetNames[0];
  const sheet = workbook.Sheets[firstSheetName];
  const rows = XLSX.utils.sheet_to_json(sheet, { defval: "" });
...
function loadMembersFromGithubExcel() {
  const url = prompt("Masukkan URL raw file Excel anggota dari GitHub:");
  if (!url) {
    return;
  }
  fetch(url)
    ...
    .then((buffer) => {
      try {
        const data = new Uint8Array(buffer);
        const workbook = XLSX.read(data, { type: "array" });
        applyWorkbookToMembers(workbook);
        showToast("Data anggota berhasil dimuat dari GitHub.");
      } catch (err) {
        showToast("Gagal membaca Excel anggota dari GitHub.");
      }
    })
    .catch(() => {
      showToast("Gagal mengunduh Excel anggota dari GitHub.");
    });
}
...
if (loadGithubExcelBtn) {
  loadGithubExcelBtn.addEventListener("click", loadBooksFromGithubExcel);
}
...
if (loadGithubMembersBtn) {
  loadGithubMembersBtn.addEventListener("click", loadMembersFromGithubExcel);
}
...
if (loadGithubLogoBtn) {
  loadGithubLogoBtn.addEventListener("click", loadLogoFromGithub);
}
